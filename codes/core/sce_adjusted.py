...
# def function(nopt, x): #(theta, masked_params, x):
#
#     return func_chi2_sce(nopt, x)
...
def cceua(func,s,sf,bl,bu,icall,maxn,iseed,testcase=True,testnr=1):
...
def sceua(func, x0,bl,bu,maxn,kstop,pcento,peps,ngs,iseed,iniflg,testcase=True,testnr=1):
...
#                10 shuffling loops
    ...
    if icall >= maxn:
        warnings.warn('*** OPTIMIZATION SEARCH TERMINATED BECAUSE THE LIMIT')
        warnings.warn('ON THE MAXIMUM NUMBER OF TRIALS ')
        warnings.warn(maxn)
        warnings.warn('HAS BEEN EXCEEDED.  SEARCH WAS STOPPED AT TRIAL NUMBER:')
        warnings.warn(icall)
        warnings.warn('OF THE INITIAL LOOP!')

    if gnrng < peps:
        warnings.warn('THE POPULATION HAS CONVERGED TO A PRESPECIFIED SMALL PARAMETER SPACE')
...
##                        idx=find(lcs(1:k3-1)==lpos)
                ...
                snew,fnew,icall=cceua(func,s,sf,bl,bu,icall,maxn,iseed,testcase=testcase,testnr=testnr)
        ...
        # Check for convergency;
        if icall >= maxn:
            warnings.warn(f'OPTIMIZATION SEARCH TERMINATED BECAUSE THE LIMIT ON THE MAXIMUM NUMBER OF TRIALS {maxn} HAS BEEN EXCEEDED.')
